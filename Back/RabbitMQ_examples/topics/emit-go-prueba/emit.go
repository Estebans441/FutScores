...
func main() {
	...
	for _, evento := range eventos {
		...
		publishMessage(ctx, ch, body, "match."+strconv.Itoa(evento.MatchID)+".event."+evento.Type)
		time.Sleep(1 * time.Second)
	}

}

func publishMessage(ctx context.Context, ch *amqp.Channel, body []byte, topic string) {
